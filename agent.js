...
/**
 * Taskify Agent — picks up issues, asks GLM-5, posts comments.
 *
 * Usage:
 *   node agent.js                          # run once (default)
 *   node agent.js --loop --interval 30     # heartbeat mode
 *
 * Options:
 *   --api URL          JSS database URL (default: http://localhost:3005/db/taskify)
 *   --agent-id ID      Agent ID (default: a1)
 *   --once             Run once and exit (default)
 *   --loop             Run in heartbeat loop
 *   --interval N       Seconds between heartbeats (default: 30)
 *
 * Requires: ZAI_API_KEY or Z_API_KEY env var (id.secret format from z.ai)
 */
...
async function run() {
  ...
  console.log('[agent] ' + agent.name + ' — run')
  ...
  if (!issue) {
    console.log('[agent] No work to do')
    agent.status = 'idle'
...
// --- Start ---
console.log('[agent] Taskify Agent ' + AGENT_ID)
console.log('[agent] API: ' + API)
console.log('[agent] Mode: ' + (LOOP ? 'loop (' + INTERVAL + 's)' : 'once'))
console.log('')
